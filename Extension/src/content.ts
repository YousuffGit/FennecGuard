...
(() => {
  ...
  document.addEventListener("input", (e) => {
    if (e.target instanceof HTMLInputElement) {
      if (e.target.type === "password") {
        ...
        if (uField && uField.value) bufferedUsername = uField.value.trim();
  ...
  function scanFields(): void {
    ...
    const usernameInputs = Array.from(document.querySelectorAll<HTMLInputElement>('input[type="text"], input[type="email"], input:not([type])'))
      .filter(input => isSecurelyVisible(input) && isUsernameField(input));

    const allInputs = [...passwordInputs, ...usernameInputs];

    allInputs.forEach(input => {
      if (input.dataset.fennecDecorated) return;
  ...
  async function attemptAutomaticAutofill(): Promise<void> {
    ...
    const matches = (data.items as VaultItemHeader[]).filter(item => {
  ...
  function isUsernameField(input: HTMLInputElement): boolean {
    if (input.disabled || input.readOnly || input.type === "hidden" || input.type === "password" || input.type === "submit") return false;
  ...
  function showNote(dropdown: HTMLElement, text: string): void {
  ...
  function renderSavePrompt(domain: string, username: string, password: string): void {
    if (document.getElementById("fennecguard-save-prompt-root")) return;
  ...
  function executeFormAutofill(username: string, password: string, originInput?: HTMLInputElement): boolean {
  ...
  function findUsernameField(referenceField?: HTMLInputElement | null): HTMLInputElement | null {
    const explicitKnown = document.querySelector<HTMLInputElement>(
      'input#login_field, input#user_login, input#username, input#email, input[name="login"], input[name="username"], input[name="email"]'
    );
    if (explicitKnown && isSecurelyVisible(explicitKnown)) return explicitKnown;

    const container = referenceField?.form || referenceField?.closest("form") || document.body;
    const candidates = Array.from(container.querySelectorAll<HTMLInputElement>('input[type="text"], input[type="email"]'))
      .filter(isSecurelyVisible);
  ...
  function isSecurelyVisible(el: HTMLInputElement): boolean {
  ...
  function isDomainMatch(targetHost: string, credentialUrl: string): boolean {
